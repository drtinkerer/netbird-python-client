...
class AccountSettings(BaseModel):
    ...
    peer_login_expiration: Optional[int] = Field(None, description="Peer login expiration in seconds")
    peer_login_expiration_enabled: bool = Field(False, description="Enable peer login expiration")
    peer_inactivity_expiration_enabled: bool = Field(False, description="Enable peer inactivity expiration")
    user_view_restrictions: Optional[Dict[str, bool]] = Field(None, description="User view restrictions")
    group_propagation_enabled: bool = Field(True, description="Enable group propagation")
    jwt_groups_enabled: bool = Field(False, description="Enable JWT groups")
    jwt_groups_claim_name: Optional[str] = Field(None, description="JWT groups claim name")
    dns_resolution_enabled: bool = Field(True, description="Enable DNS resolution")
    network_traffic_logging_enabled: bool = Field(False, description="Enable network traffic logging")
