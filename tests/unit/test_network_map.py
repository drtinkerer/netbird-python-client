...
def mock_api_responses():
    ...
    return {
        'networks': [
            {'id': 'net-1', 'name': 'Network 1', 'resources': ['res-1'], 'routers': ['rtr-1'], 'policies': ['pol-1']},
            {'id': 'net-2', 'name': 'Network 2', 'resources': ['res-2'], 'routers': [], 'policies': []}
        ],
        'resources': {
            'net-1': [
                {
...
class TestGenerateFullNetworkMap:
    ...
    def test_generate_full_network_map_success(self, mock_api_responses):
        ...
        mock_client.networks.list_resources.side_effect = lambda net_id: mock_api_responses['resources'].get(net_id, [])
        mock_client.networks.list_routers.side_effect = lambda net_id: mock_api_responses['routers'].get(net_id, [])
        mock_client.policies.list.return_value = mock_api_responses['policies']
        mock_client.policies.get.side_effect = lambda pol_id: next((p for p in mock_api_responses['policies'] if p['id'] == pol_id), None)
    ...
    def test_generate_full_network_map_with_options(self, mock_client, mock_api_responses):
    ...
    def test_generate_full_network_map_empty_resources(self):
        ...
        mock_client = Mock()
        mock_client.networks.list.return_value = [{'id': 'net-1', 'name': 'Empty Network', 'resources': [], 'routers': [], 'policies': []}]
...
class TestGetNetworkTopologyData:
                ...
                'policies': [
                    {
                        ...
                        'rules': [
                            {
                                'sources': [{'id': 'grp-3', 'name': 'developers'}],
                                'destinations': [{'id': 'grp-1', 'name': 'web-servers'}],
                                'destinationResource': {}
                            },
                            {
                                'sources': [{'id': 'grp-3', 'name': 'developers'}], 
    ...
    def test_get_topology_data_include_options(self):
        ...
        with patch('netbird.network_map.generate_full_network_map') as mock_generate:
            ...
            result = get_network_topology_data(
                mock_client, 
        ...
        for net_idx in range(10):
            ...
            for pol_idx in range(50):
                policies.append({
                    ...
                    'rules': [
                        {
                            'sources': [{'id': f'grp-src-{pol_idx % 3}', 'name': f'source-{pol_idx % 3}'}],
                            'destinations': [{'id': f'grp-{pol_idx % 5}', 'name': f'group-{pol_idx % 5}'}],
                            'destinationResource': {}
                        }
                    ]
                })
